...
# from ADVENT.advent.model.deeplabv2 import ResNetMulti
AFFINE_PAR = True
...
class ResNetMultiDepth(ResNetMulti):
    def __init__(self, block, layers, num_classes, multi_level):
        ...
        # self.deConv = nn.ConvTranspose2d(128, 16, kernel_size=3, stride=2, padding=1, output_padding= 1,bias=True)
        self.enc4_4 = nn.Conv2d(128, 16, kernel_size=1, stride=1, padding=0, bias=True)
        self.dec4 = nn.Conv2d(128, 2048, kernel_size=1, stride=1, padding=0, bias=True)

        self.enc5_1 = nn.Conv2d(2048, 512, kernel_size=1, stride=1, padding=0, bias=True)
        self.enc5_2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=True)
        self.enc5_3 = nn.Conv2d(512, 128, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        density = torch.mean(x4_enc, dim=1, keepdim=True)  # density output

        x5_enc = self.enc5_1(x4)
        x5_enc = self.relu(x5_enc)
        x5_enc = self.enc5_2(x5_enc)
        x5_enc = self.relu(x5_enc)
        x5_enc = self.enc5_3(x5_enc)
        depth = torch.mean(x5_enc, dim=1, keepdim=True)  # depth output

        seg_conv5 = self.layer6(x4)  # seg
        return seg_conv4, seg_conv5, density, depth
    ...
    def get_10x_lr_params(self):
        ...
        b.append(self.enc5_1.parameters())
        b.append(self.enc5_2.parameters())
        b.append(self.enc5_3.parameters())
...
def get_deeplab_v2_depth(num_classes=16):
    model = ResNetMultiDepth(
        Bottleneck, [3, 4, 23, 3], num_classes, False
    )
    return model
