...
def processPic_1(fileLabel, fileDepth, index, name, path):
    size = (1280, 760)
    rec = _load_img(fileLabel, size, Image.NEAREST, rgb=False)
    id_to_train = [3,4,2,21,5,7,15,9,6,1,10,17,8,19,12,11]
    rec = (rec == id_to_train[index])

    dep = get_depth(fileDepth)
    depA = dep * rec
    ccImage = Image.fromarray(depA.astype('float'))
    ccout1 = ccImage.resize((161, 96), Image.NEAREST)  # resize image with high-quality
    depA_resize = np.array(ccout1)
    ...
    a = nplist
    ccImage = Image.fromarray(a.astype('float'))
    ccImage.save('{}/{}-{}.tiff'.format(path, name, index))

def processPic_2(fileLabel, fileDepth, index, name, path):
    size = (1280, 760)
    rec = _load_img(fileLabel, size, Image.NEAREST, rgb=False)

    if index == 0:
        rec = ((rec == 3) |(rec == 4) | (rec == 22))
    elif index == 1:
        rec = ((rec == 2) |(rec == 5))
    elif index == 2:
        rec = ((rec == 7) |(rec == 9) | (rec == 15))
    elif index == 3:
        rec = (rec == 6)
    elif index == 4:
        rec = (rec == 1)
...
def processPic_3(fileLabel, fileDepth, index, name, path):
    size = (1280, 760)
    rec = _load_img(fileLabel, size, Image.NEAREST, rgb=False)
    ...
    depA = dep * rec
    ccImage = Image.fromarray(depA.astype('float'))
    ccout1 = ccImage.resize((81, 41), Image.NEAREST)  # resize image with high-quality
    depA_resize = np.array(ccout1)
...
def main():
    ...
    basePath_fileLabel = cfg.DATA_DIRECTORY_SOURCE + '/parsed_LABELS/'
    basePath_fileDepth = cfg.DATA_DIRECTORY_SOURCE + '/Depth/'
    fileLabels = os.listdir(basePath_fileLabel)
    count = 0
    for item in fileLabels:
        fileLabel = basePath_fileLabel + item
        fileDepth = basePath_fileDepth + item
        name = os.path.basename(fileLabel)
        print('filename:', name)
        name1 = name.replace('.png', '')
        start = time.time()
        count += 1

        for i in range(cfg.NUM_CLASSES):
            processPic(fileLabel, fileDepth, i, name1, args.output_dir)
...
def get_arguments():
    ...
    parser.add_argument('--output-dir', type=str,  default='../../data/SYNTHIA/source_density_maps', help='folder where pseudo labels are stored')
