...
def main():
    ...
    if cfg.TRAIN.MODEL == "DeepLabv2_depth":
        ...
        if args.pret_model != '':
            iternum, model_adv_name = split_premodelname(args.pret_model)
            saved_state_dict = torch.load(args.pret_model)
            saved_state_dict_adv = torch.load(model_adv_name)
            model.load_state_dict(saved_state_dict['model'])
            d_main.load_state_dict(saved_state_dict_adv['model'])
        else:
    ...
    if args.pret_model != '':
        ...
        optimizer_d_main.load_state_dict(saved_state_dict_adv['optimizer'])
    ...
    train_depdis(model, d_main, optimizer, optimizer_d_main, source_loader, target_loader,test_loader, cfg, expid, iternum)
