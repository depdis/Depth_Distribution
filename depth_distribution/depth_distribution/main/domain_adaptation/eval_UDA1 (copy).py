...
def evaluate_domain_adaptation( models, test_loader, cfg,restorm_from,fixed_test_size=True,verbose=True):
    device = cfg.GPU_ID
    interp = None
    if fixed_test_size:
...
def eval_best(cfg, models,
    ...
    cur_best_miou = -1
    cur_best_model = ''
    for i_iter in range(1000,90000, 1000):#range(start_iter, max_iter + 1, step):
